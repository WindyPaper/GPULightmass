...
{	
	...
	GPULightmass::float3 vertex_pos[vertices_num];
	vertex_pos[0] = GPULightmass::make_float3(0, -50, -50);
	vertex_pos[1] = GPULightmass::make_float3(0, 50, 50);
	vertex_pos[2] = GPULightmass::make_float3(0, -50, 50);

	GPULightmass::float2 uvs[vertices_num];
	uvs[0] = GPULightmass::make_float2(0, 0);
	uvs[1] = GPULightmass::make_float2(1, 1);
	uvs[2] = GPULightmass::make_float2(0, 1);

	GPULightmass::int3 index[triangle_num];
	index[0] = GPULightmass::make_int3(0, 1, 2);

	GPULightmass::float3 bbox[2];
	bbox[0] = GPULightmass::make_float3(0, -50, -50);
	bbox[1] = GPULightmass::make_float3(0, 50, 50);

	const int surfel_num = 100;
	GPULightmass::SurfelData *pSurfData = new GPULightmass::SurfelData[surfel_num];
	int size = sizeof(GPULightmass::SurfelData) * surfel_num;
